...
## Message 171 [2026-03-24]

遇到两个问题，1. 现在interactive-captcha模式通过tmux ssh连接到服务器时，仍然存在粘贴后卡死的情况，只有用ctrl c终止程序才行。2. 我发现每次开始获取引用时都要输入验证码，我们的工作pattern是不是有什么问题？比如在从profile跳转到citation的时候。

---

## Message 172 [2026-03-24]

确认

---

## Message 173 [2026-03-24]

遇到两个问题，1. 现在interactive-captcha模式通过tmux ssh连接到服务器时，仍然存在粘贴后卡死的情况，只有用ctrl c终止程序才行。2. 我发现每次开始获取引用时都要输入验证码，我们的工作pattern是不是有什么问题？比如在从profile跳转到citation的时候。

---

## Message 174 [2026-03-24]

我测试了一下，粘贴之后没有响应的问题仍然存在

---

## Message 175 [2026-03-24]

怎么能要求用户在2s内完成粘帖？来不及操作怎么办？换一个别的思路。

---

## Message 176 [2026-03-24]

timeout时间太短了，建议设为3s

---

## Message 177 [2026-03-24]

还有bug，复制的内容并没有完全被接受，即使等了3s也是一样。感觉应该不是传输速度的问题。

---

## Message 178 [2026-03-24]

不行，不能粘贴的话，我怎么把文件放到远程服务器上？还是设计一个粘贴可以搞定的方案。

---

## Message 179 [2026-03-24]

我测试一下

---

## Message 180 [2026-03-24]

不行，我粘贴进去之后只显示了几行，然后就无法操作了。

---

## Message 181 [2026-03-24]

之前readline的方案好像是可以的，虽然没有显示粘贴的内容，但是显示了粘贴了x行，要不再试试这个方案？请单独写个脚本先测试好了再更新整个项目。

---

## Message 182 [2026-03-24]

第二种好像可以

---

## Message 183 [2026-03-24]

又发现一个问题，probing year失败为什么没有引起填写验证码的过程？应该统一到一样的流程中去才对吧。

---

## Message 184 [2026-03-24]

另外，probing year失败之后，再重新尝试，起始年份从2017变成了2022，也请检查一下出了什么问题。

---

## Message 185 [2026-03-24]

probe的结果也不应该是看固定档位吧，页面上其实有显示逐年的引用信息，那些档位只能作为参考。

---

## Message 186 [2026-03-24]

确认

---

## Message 187 [2026-03-24]

出现验证码提示之后是不是应该考虑原地重试？如果是向外抛出异常的话，再恢复原先的工作状态就略显复杂。你觉得呢？

---

## Message 188 [2026-03-24]

比如从17年开始收集引用，到22年提示要输入验证码。输入完之后，又再probe year是不是有点不太对？

---

## Message 189 [2026-03-24]

在probe处是这样的，其他需要验证的地方也应该是类似的，如果可以输入验证码，就可以最低代价恢复。

---

## Message 190 [2026-03-25]
...
## Message 197 [2026-03-26]

我们确认一下年份管理的逻辑，是否每次抓取都会做一次年份的确认呢？因为有可能会因为数据源问题新获取一个之前年份的引用。并且这个代价也不太大。

---

## Message 198 [2026-03-26]

好的，我同意上面关于probe的方案

---

## Message 199 [2026-03-26]

提醒一下，我有个conda环境，叫做scholar

---

## Message 200 [2026-03-26]

好的，可以更新了

---

## Message 201 [2026-03-26]

你对刚刚整理的force refresh citation的逻辑有什么建议吗？

---

## Message 202 [2026-03-26]

因为原先对年份重扫的选项是由hard控制的，现在已经默认会重新获取了，所以也不需要这个控制了。--skip和--limit没问题，就保持现在的语义。force-refresh是不是需要改个名字？

---

## Message 203 [2026-03-26]

好的，可以执行这个修改吧。

---

## Message 204 [2026-03-26]

1

---

## Message 205 [2026-03-26]

确认

---

## Message 206 [2026-03-26]

你在干啥，怎么这么慢

---

## Message 207 [2026-03-26]
...
## Message 232 [2026-03-28]

另一个问题，interactive模式每40次左右页面访问就要输入一次验证码，等待更多时间并没有什么用处。请把间隔时间放短一点，比如变成现在的1/10，然后我再试一试。

---

## Message 233 [2026-03-28]

为了测试这个事情的效果，请在全局状态里加一个被验证的次数，也一起在输出信息的时候输出出来。

---

## Message 234 [2026-03-28]

profile 阶段本身就不容易被block，也要缩短

---

## Message 235 [2026-03-28]

好的，更新和提交吧

---

## Message 236 [2026-03-28]

频率太高了，把这个变成一个参数吧，比如--accelerate 0.1 表示快10倍，也就是现在这个速度，如果参数是1或者不加，就是原先的结果。

---

## Message 237 [2026-03-28]
