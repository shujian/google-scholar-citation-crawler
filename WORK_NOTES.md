...
- **2026-04-05** — 修复 year-based progress save 重复触发：此前在把 `_fetch_by_year()` 从“每 `save_every` 条 citation 保存一次”切到“每页保存一次”后，又保留了年份结束时的兜底保存，导致同一页若正好是该年份最后一页，会连续触发两次 `save_progress(complete=False)`；同时 `_SearchScholarIterator` 的 `_finished_current_page` 在页末可能连续保持为 `True`，也会放大重复保存现象。修复：year fetch 循环内新增 `page_save_emitted`，确保同一页边界只保存一次；按年份维度新增 `year_progress_saved`，如果该年最后一页已经做过页末保存，则年份结束时不再重复保存。对应回归测试 `test_fetch_by_year_finishes_current_page_before_stopping()` 现固定断言 save 序列为 `[False, True]`，`python -m unittest test_citation_page_stop.py` 通过（34 tests）。
