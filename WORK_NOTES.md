...
- **2026-04-04** — 修复 citation year probe 的双重等待：`_probe_citation_start_year()` 过去会先自己执行一次 `rand_delay/time.sleep` 并打印 `Probing citation year range (... wait)...`，随后 `nav._get_soup(...)` 又会走 `_patch_scholarly()` 中 `patched_get_page()` 的统一 pre-request wait，导致同一次 probe request 在真正访问页面前被等待两次、日志也显得像“没有访问页面却又在等”。现已删除 probe 本地等待，让 probe 请求只复用统一的 `Waiting ... before request...` 逻辑；保留 referer、`_current_attempt_url`、captcha/proxy-switch/retry 和 histogram 解析行为不变。`test_citation_page_stop.py` 新增回归测试，确认 `_probe_citation_start_year()` 本身不再直接 sleep，也不再输出旧的 probe-wait 文案；`python -m unittest test_citation_page_stop.py` 通过（30 tests）。
