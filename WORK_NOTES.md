...
- **2026-03-20** — 交互式验证码协助：程序在遇到 captcha/403 时（需 `--interactive-captcha` 参数），打印失败 URL 并提示用户在浏览器中打开该页面解决验证码，然后通过 DevTools 导出 cURL 粘贴到控制台；程序解析 cURL 中的 `-b 'cookie_str'`，将所有 cookie 注入到 scholarly 的两个 httpx session，并立即重试（跳过原来的 6h 等待）；不加参数时默认禁用（安静等待）
