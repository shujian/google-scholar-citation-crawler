...
- **2026-04-17** — 修复 `while True` 循环在短页成功后无谓重试的问题：`_SearchScholarIterator` 在返回最后一个短页（< SCHOLAR_PAGE_SIZE）的 items 后，会自动尝试请求下一页；如果下一页 load 抛出异常（captcha / 限流），except 处理器会误判为 captcha retry，重新构造从当前 page start 开始的 iterator，形成不必要的二次循环。修复方式：在 for 循环里追踪 `last_completed_page_was_short`（在 `_finished_current_page` 为 True 时记录上一页的 item 数），在 except 处理器里新增判断：若 `last_completed_page_was_short` 且当前 `_items_in_current_page == 0`（异常发生在新页 load 开始前），直接 break 并设置 `short_page_stop`，不触发 captcha retry。保留 `while True` 只用于真实 captcha retry（全页 + 异常场景）。新增回归测试 `test_short_page_exception_at_next_page_does_not_retry`。95 tests OK。
